...
subcategory: "manage"
layout: "nd"
page_title: "ND: nd_fabric_vxlan_ibgp"
sidebar_current: "docs-nd-resource-nd_fabric_vxlan_ibgp"
description: |-
  
---

# nd_fabric_vxlan_ibgp #



## API Information ##
* Fabric Vxlan Ibgp Management API information is not defined in the YAML.
...
The configuration snippet below shows all possible attributes of the Fabric Vxlan Ibgp resource.

!> This example might not be valid configuration and is only used to show all possible attributes.

Example source: `examples/resources/nd_fabric_vxlan_ibgp/resource.tf`
...
All examples for the Fabric Vxlan Ibgp resource can be found in the [examples](https://github.com/CiscoDevNet/terraform-provider-nd/tree/main/examples/resources/nd_fabric_vxlan_ibgp) folder.

## Schema ##

### Required ###
* `fabric_name` (name) - (String) The name of the fabric_vxlan_ibgp resource
...
* `bootstrap_subnet_collection` (bootstrapSubnetCollection) - (List) Bootstrap subnet collection for switch POAP
    * `start_ip` (startIp) - (String) Start IP address of the bootstrap subnet scope
    * `end_ip` (endIp) - (String) End IP address of the bootstrap subnet scope
    * `default_gateway` (defaultGateway) - (String) Default gateway for the bootstrap subnet scope
    * `subnet_prefix` (subnetPrefix) - (Int64) Subnet prefix length for the bootstrap subnet scope
...
* `dlb_mode` (dlbMode) - (String) DLB mode. Valid values are flowlet, per-packet, mixed-mode
* `dlb_mixed_mode_default` (dlbMixedModeDefault) - (String) Default load balancing mode for mixed-mode DLB. Valid values are ecmp, flowlet, per-packet
* `flowlet_aging` (flowletAging) - (Int64) Flowlet aging timer in microseconds
* `roce_v2` (rocev2) - (String) RoCEv2 DSCP value for AI/ML QoS
* `cnp` (cnp) - (String) CNP DSCP value for AI/ML QoS
* `wred_min` (wredMin) - (Int64) WRED minimum threshold for AI/ML QoS
* `wred_max` (wredMax) - (Int64) WRED maximum threshold for AI/ML QoS
* `wred_drop_probability` (wredDropProbability) - (Int64) WRED drop probability for AI/ML QoS
* `wred_weight` (wredWeight) - (Int64) WRED weight for AI/ML QoS
* `bandwidth_remaining` (bandwidthRemaining) - (Int64) Bandwidth remaining percentage for AI/ML QoS
...
* `security_group_tag_mac_segmentation` (securityGroupTagMacSegmentation) - (Bool) Enable SGT MAC Segmentation on all switches in the fabric
...
An existing Fabric Vxlan Ibgp resource can be imported with its identifier via the following command:

```shell
terraform import nd_fabric_vxlan_ibgp.example {id}
```

Starting in Terraform version 1.5, an existing Fabric Vxlan Ibgp resource can be imported using import blocks via the following configuration:

```hcl
import {
  id = "{id}"
  to = nd_fabric_vxlan_ibgp.example
}
```
