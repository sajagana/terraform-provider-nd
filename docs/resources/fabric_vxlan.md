...
subcategory: "manage"
layout: "nd"
page_title: "ND: nd_fabric_vxlan"
sidebar_current: "docs-nd-resource-nd_fabric_vxlan"
description: |-
...
## API Information ##
* Fabric Vxlan Management API information is not defined in the YAML.
...
The configuration snippet below shows all possible attributes of the Fabric Vxlan resource.

!> This example might not be valid configuration and is only used to show all possible attributes.

Example source: `examples/resources/nd_fabric_vxlan/resource.tf`
...
All examples for the Fabric Vxlan resource can be found in the [examples](https://github.com/CiscoDevNet/terraform-provider-nd/tree/main/examples/resources/nd_fabric_vxlan) folder.

## Schema ##

### Required ###
* `fabric_name` (name) - (String) The name of the fabric_vxlan resource
...
An existing Fabric Vxlan resource can be imported with its identifier via the following command:

```shell
terraform import nd_fabric_vxlan.example {id}
```

Starting in Terraform version 1.5, an existing Fabric Vxlan resource can be imported using import blocks via the following configuration:

```hcl
import {
  id = "{id}"
  to = nd_fabric_vxlan.example
}
```
