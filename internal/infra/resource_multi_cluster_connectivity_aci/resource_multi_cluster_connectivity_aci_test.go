...
func TestResourceMultiClusterConnectivityAci(t *testing.T) {
	type args struct {
		rscType  string
		rscName  string
		dataFile string
	...
	tests := []struct {
		...
		{
			name: "Test_resource_multi_cluster_connectivity_aci",
			args: args{
				rscType:  "resource",
				rscName:  "multi_cluster_connectivity_aci",
				dataFile: "/examples/ndfc_payloads/data_multi_cluster_connectivity_aci.json",
			},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name+"model_read", func(t *testing.T) {
			...
			modelData := NDFCMultiClusterConnectivityAciModel{}
			v := MultiClusterConnectivityAciModel{}
